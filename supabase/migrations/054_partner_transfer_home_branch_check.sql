-- ============================================================
-- 054: home_branch_id check в create_partner_transfer
-- ============================================================
-- Дырка в роле-модели (баг A1 из аудита):
--   attach_transfer_to_partner (047) уже проверяет: accountant
--   не может прикреплять перевод к партнёру с home_branch_id
--   другого филиала (через миграцию 046). Но create_partner_transfer
--   (047) этой проверки НЕ делает — accountant мог создать
--   партнёрский перевод через ЛЮБОГО партнёра, даже если тот
--   привязан к чужому филиалу. Закрываем.
--
-- Доп.: переименовываем v_role/v_assigned + добавляем check
-- что from_branch_id ∈ assigned_branch_ids у accountant'а
-- (исторически было неявно, теперь явная проверка для симметрии
-- с attach_transfer_to_partner).
--
-- Note: A2 (to_account_id = '' → NULL) НЕ включён сюда — поле
-- объявлено `text NOT NULL DEFAULT ''` в 001, NULL отвергнет
-- constraint, а ALTER COLUMN DROP NOT NULL затронет и другие
-- RPC, которые читают это поле как sentinel «no account».
-- Делать отдельной миграцией, после ручного аудита всех мест.
--
...
BEGIN
  ...
  -- ── Home-branch check (баг A1: accountant не должен создавать
  -- партнёрский перевод через партнёра, привязанного к чужому филиалу).
  -- Зеркало проверки в attach_transfer_to_partner (миграция 046).
  ...
  -- Saldo в валюте перевода (как в 047, без сворачивания в base).
