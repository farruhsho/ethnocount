-- ============================================================
-- 082: Починка approval «изменить сумму перевода» (transfer_amend_amount)
-- ============================================================
-- ПРОБЛЕМА (ре-аудит 07.2026, HIGH) — действие было сломано ВДВОЙНЕ:
--
--  (1) СИГНАТУРА. Диспатч в approve_request (021 и 076:234) вызывает
--      private.update_transfer_amount(uuid, float8, text) — три позиционных
--      аргумента, где note попадает в слот p_new_exchange_rate. Реальная
--      сигнатура (071) — (uuid, float8, float8 DEFAULT, text DEFAULT).
--      Неявного каста text→float8 в Postgres нет → резолвинг падает
--      «function does not exist», approve откатывается, заявка ВЕЧНО pending
--      (uq_pending_approvals_target_action_pending не даёт пересоздать).
--
--  (2) ГЕЙТ СТАТУСА. UI создаёт transfer_amend_amount ТОЛЬКО для НЕ-created
--      переводов (edit_transfer_dialog.dart:201 — created бухгалтер правит
--      напрямую через replace_pending_transfer). А update_transfer_amount
--      (071) принимает ТОЛЬКО status='created'. Значит даже с исправленной
--      сигнатурой approve всегда падал бы «Only created can be amended».
--      Тупик: изменить сумму принятого перевода нельзя было ни бухгалтеру
--      (approval мёртв), ни директору/создателю (прямой путь тоже бьёт в
--      created-гейт).
--
-- РЕШЕНИЕ:
--   • Расслабляем гейт до ЛЮБОГО недоставленного перевода, по которому
--     ещё НЕ было выдач и который НЕ привязан к партнёру. Под net-0 это
--     безопасно: касса-отправитель дебетуется при создании и НЕ трогается
--     на confirm; получатель кредитуется только при выдаче (issue). Пока
--     перевод в created/toDelivery/withCourier и issued_amount=0 — ре-баланс
--     кассы-источника на дельту (что и делает функция) корректен.
--       – delivered — запрещаем (деньги уже выданы, сумму не переписать);
--       – issued_amount>0 — запрещаем (частичная выдача уже прошла);
--       – via_counterparty_id — запрещаем (у привязанного перевода дебет
--         кассы откачен partner_offset'ом 056, ре-баланс был бы неверным;
--         менять сумму через detach → правку → re-attach).
--   • Добавляем ПРИВАТНЫЙ 3-арг шим update_transfer_amount(uuid,float8,text),
--     на который однозначно резолвится существующий диспатч (3-арг точное
--     совпадение; 4-арг кандидатом не является — text не кастуется в float8).
--     Шим делегирует в каноничную 4-арг версию с курсом NULL (amend меняет
--     только сумму, курс сохраняется). Шим в схеме private → PostgREST его
--     НЕ экспонирует, прямой REST-путь Dart (именованные 4 арг, transfer_
--     remote_ds.dart:435) продолжает бить в каноничную функцию. Так мы НЕ
--     переписываем большой approve_request и не рискуем его дрейфом.
--
-- Идемпотентно: CREATE OR REPLACE. Логика/уведомления 071 сохранены 1:1,
-- изменён ТОЛЬКО блок гейта статуса.
-- ============================================================

BEGIN;

-- ── 1. Каноничная функция: расслабленный гейт ──
...
BEGIN
  ...
  -- ── 082: расслабленный гейт (было: только 'created') ──
...
-- ── 2. Приватный 3-арг шим для диспатча approve_request ──
-- Диспатч (021/076) исторически зовёт update_transfer_amount(uuid,float8,text).
-- Этот шим — точное совпадение для такого вызова; каноничная 4-арг версия
-- кандидатом НЕ является (text не кастуется в float8). Делегируем в неё с
-- курсом NULL (amend курс не меняет). Живёт в private → REST не экспонирует.
CREATE OR REPLACE FUNCTION private.update_transfer_amount(
  p_transfer_id uuid,
  p_new_amount double precision,
...
SET search_path = public
AS $$
  SELECT private.update_transfer_amount(
    p_transfer_id, p_new_amount, NULL::double precision, p_amendment_note
  );
$$;

-- Шим вызывается только изнутри private.approve_request (SECURITY DEFINER,
-- под владельцем) — прямой доступ роли authenticated не нужен и не даётся.
REVOKE EXECUTE ON FUNCTION
  private.update_transfer_amount(uuid, double precision, text)
  FROM PUBLIC;
