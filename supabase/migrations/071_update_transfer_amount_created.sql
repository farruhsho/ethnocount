-- ============================================================
-- 071: update_transfer_amount — починка гейта статуса
-- ============================================================
-- Контекст: update_transfer_amount (последняя редакция в
-- 014_issuance_account_amend_notify.sql) всё ещё проверял
-- `v_t.status != 'pending'`. Но 022_status_redesign.sql переименовал
-- состояние «ожидает» pending → created. Из-за этого правка суммы
-- любого «created»-перевода падала с ошибкой
-- «Only pending transfers can be amended (current: created)».
--
-- Что меняется: ТОЛЬКО гейт статуса. Принимаем редактируемое
-- состояние 'created' — ровно так же, как gating в
-- replace_pending_transfer (024_full_edit_pending_transfer.sql:71
-- `IF v_t.status <> 'created'`). Вся остальная логика и уведомления
-- идентичны редакции 014.
--
-- Идемпотентно. CREATE OR REPLACE (та же сигнатура). Re-GRANT.
-- ============================================================
...
BEGIN
  ...
  IF v_t.status <> 'created' THEN
    RAISE EXCEPTION 'Only created transfers can be amended (current: %)', v_t.status;
  END IF;
