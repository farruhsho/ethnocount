...
export const seedBranches = onCall(async (request) => {
    const { auth } = request;
    if (!auth) {
    ...
    if (!userDoc.exists || (role !== 'creator' && role !== 'admin')) {
        throw new HttpsError('permission-denied', 'Only creator can seed branches.');
    }
    ...
    if (!existingBranches.empty) {
        throw new HttpsError(
            'already-exists',
            `Branches already exist (${existingBranches.size} found). Seed aborted to prevent duplicates.`,
        );
    }

    const batch = db.batch();
    const created: { branchId: string; name: string; accountCount: number }[] = [];
    ...
    batch.set(auditRef, {
        ...
        details: { branchCount: BRANCHES.length, totalAccounts: BRANCHES.reduce((s: number, b) => s + b.accounts.length, 0) },
        createdAt: admin.firestore.FieldValue.serverTimestamp(),
    });
